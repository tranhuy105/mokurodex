...
interface MangaCardProps {
  manga: MangaMetadata & { userData?: UserMangaMetadata | null };
}
...
function getStatusIcon(status: string) {
  switch (status) {
    ...
    case "completed":
      return <CircleCheck className="w-3 h-3 text-blue-500" />;
    case "on-hold":
      return <PauseCircle className="w-3 h-3 text-orange-500" />;
    case "dropped":
      return <BookmarkX className="w-3 h-3 text-red-500" />;
    case "plan-to-read":
        ...
        {manga.coverImage ? (
          <Image
            ...
            className={`object-cover transition-transform group-hover:scale-105 duration-300 ease-out ${
              isNsfw && !isRevealed ? "blur-xl filter" : ""
            }`}
          />
        ) : (
        ...
        <div className="flex flex-wrap gap-1.5 mt-auto pt-2">
          {manga.userData?.status && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300">
              {getStatusIcon(manga.userData.status)}
              <span>{manga.userData.status.replace(/-/g, " ")}</span>
            </span>
          )}
