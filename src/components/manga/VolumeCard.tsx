...
export const VOLUME_MOCK_DATA = {
    ...
    getReleaseInfo: (seed: string) => {
        ...
        const publisherIndex =
            Math.abs(hash) % publishers.length;
...
export function VolumeCard({
    ...
    const [isHoveringPreview, setIsHoveringPreview] =
        useState(false);
    ...
    useEffect(() => {
        async function checkNsfwStatus() {
            try {
                const userData = await getUserMangaMetadata(
                    mangaId
                );
                if (userData?.isNsfw) {
                    setIsNsfw(true);
    ...
    const releaseInfo =
        hasMetadata && volume.metadata
            ? {
                  publisher:
                      volume.metadata.publisher ||
                      "Unknown Publisher",
                  releaseDate: formatDate(
                      volume.metadata.releaseDate
                  ),
              }
    ...
    const targetPage = lastReadEntry
        ? lastReadEntry.page
        : 1;
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 transition-all hover:shadow-lg h-full">
            <div className="sm:flex h-full">
                ...
                <div className="sm:w-2/5 lg:w-64 xl:w-72 flex-shrink-0 flex">
                    <Link
                        href={`/manga/${encodeURIComponent(
                            mangaId
                        )}/${
                            volume.mokuroData.volume_uuid
                        }/${targetPage}`}
                        className="group block relative w-full h-full bg-gray-100 dark:bg-gray-900 sm:min-h-[280px]"
                        onClick={
                    ...
                    >
                        {!contentReady ? (
                            ...
                            <>
                                ...
                                {/* Volume badge */}
                                <div className="hidden md:block absolute top-3 left-3 bg-orange-600 text-white px-2.5 py-1 rounded-md text-sm font-bold shadow-md z-30">
                                    Vol.{" "}
                                    {
                                        volume.mokuroData
                                            .volume
                ...
                {/* Content section */}
                <div className="sm:w-3/5 lg:w-2/3 p-3 sm:p-4 flex flex-col justify-between h-full mt-3 sm:mt-0">
                    ...
                    <div>
                        ...
                        {/* Volume metadata */}
                        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-x-3 gap-y-2 text-sm text-gray-600 dark:text-gray-300 mb-3">
                    ...
                    {/* Preview section */}
                    <div className="border-t border-gray-200 dark:border-gray-700 pt-3 flex-grow flex flex-col">
                        ...
                        {hasPreviewImages ? (
                            <div className="sm:flex sm:space-x-3 flex-grow">
                                {/* Main preview image - contained size on desktop */}
                                <div
                                    className="relative w-full sm:w-3/4 aspect-[4/3] sm:aspect-[3/2] bg-gray-100 dark:bg-gray-900 rounded-lg overflow-hidden"
                                    onMouseEnter={() =>
                          ...
                          ${
                              isHoveringPreview
                                ...
                                <div className="mt-2 sm:mt-0 sm:w-1/4 flex flex-wrap sm:grid sm:grid-cols-2 gap-1">
                                    {contentReady
                                        ? previewImages
                                              .slice(0, 8)
                                              .map(
