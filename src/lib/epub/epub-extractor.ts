...
export async function extractEpubMetadata(
    ...
    try {
        ...
        const coverImagePath = await extractCoverImage(
            zip,
            coverOutputDir
        );
...
async function extractBasicMetadata(
    ...
    try {
        ...
        const containerFile =
            zip.files["META-INF/container.xml"];
        if (!containerFile) {
            throw new Error(
        ...
        const containerXml = await containerFile.async(
            "text"
        );
...
async function extractCoverImage(
    ...
    try {
        ...
        const coverFromMetadata =
            await findCoverFromMetadata(zip);
        if (
            coverFromMetadata &&
        ...
        const coverCandidates = Object.keys(
            zip.files
        ).filter((path) => {
        ...
        const sortedCandidates = coverCandidates.sort(
            (a, b) => {
        ...
        if (allImages.length > 0) {
            ...
            const likelyCovers = allImages.filter(
                (path) => {
                    const fileName =
                        path
                            .split("/")
 ...
 */
async function findCoverFromMetadata(
    zip: JSZip
): Promise<string | undefined> {
    try {
        // Parse container.xml to find OPF file
        ...
        console.warn(
            "Failed to find cover from metadata:",
            error
        );
        return undefined;
    }
...
async function saveCoverImage(
    ...
    try {
        ...
        // Use a timestamp to ensure uniqueness of cover files
        const timestamp = Date.now();
        const coverFileName = `cover_${timestamp}${originalExtension}`;
