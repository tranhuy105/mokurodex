...
export function getMangaReaderProps({
  ...
  return {
    ...
    pages,
    initialPage,
    initialTimestamp: Date.now(), // Add timestamp to help with state management
  };
}
 ...
 */
export function dispatchRightToLeftChangedEvent(rightToLeft: boolean): void {
...
export function navigateToPage(
  ...
  const url = getMangaPageUrl(mangaId, volumeId, pageNumber);
...
export function parseMangaReaderUrl(url: string): {
  ...
  try {
    ...
    if (segments.length >= 4 && segments[0] === "manga") {
      return {
    ...
    console.error("Failed to parse manga reader URL:", error);
    return {
      mangaId: null,
      volumeId: null,
      page: null,
